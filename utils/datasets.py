...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride
        self.path = path
        self.albumentations = Albumentations(self.hyp) if augment else None
        ...
        class_count = np.histogram(np.concatenate(labels)[:, 0], bins=int(np.concatenate(labels)[:, 0].max()+1))[0]
        self.less_class = [i[0] for i in enumerate(class_count < np.mean(class_count)) if i[1]]
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # Rotate 90 degree
            if 'rot90' in hyp and random.random() < hyp['rot90']:
                p = random.random()
                k = 1 if p < 0.5 else 3
                img = np.rot90(img, k)
                if nl:
